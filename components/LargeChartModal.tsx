...
export const LargeChartModal: React.FC<LargeChartModalProps> = ({ ticker, onClose }) => {
    return (
        <div 
        ...
        >
            ...
            >
                <div className="flex justify-between items-center mb-2 flex-shrink-0">
                    <h2 className="text-xl font-bold text-cosmic-text-primary">Chart: {ticker}</h2>
                    <button onClick={onClose} className="text-cosmic-text-secondary hover:text-cosmic-text-primary">
                        <XIcon className="w-6 h-6" />
                    </button>
                </div>
                <div className="flex-grow w-full h-full">
                    <EmbeddedStockChart ticker={ticker} />
                </div>
            </div>
        </div>
    );
